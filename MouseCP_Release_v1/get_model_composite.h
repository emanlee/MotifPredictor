...
void get_model( struct pairnode * miRNA2tfnodes,   long row_count_1,
				...
				struct pairnode * tf2miRNAnodes,   long row_count_4,
				char  output_file[]
				)
{
	/* 
	...
	*/
	...
	long fllcount=0, xx, yy, zz, kk,  found=0;
	...
	{
		// check  exist: tf2miRNA
		found=0;
		for(kk=0; kk<row_count_4; kk++)
		{
			if(strcmp(miRNA2tfnodes[xx].left,  tf2miRNAnodes[kk].right)==0 
			&& strcmp(miRNA2tfnodes[xx].right, tf2miRNAnodes[kk].left)==0)
				found=1;
		}
		if(found==0) continue;
					 ...
					 )
					 {
						 printf ("%s~%s, %s~%s, %s~%s, %s~%s\n", 
							 ...
							 miRNA2genenodes[yy].left, miRNA2genenodes[yy].right,
							  miRNA2tfnodes[xx].right, miRNA2tfnodes[xx].left
							  )	;
						  fllcount++;
						  
						  	fprintf(fid_output, "%s~%s, %s~%s, %s~%s, %s~%s\n", 
							 miRNA2tfnodes[xx].left, miRNA2tfnodes[xx].right,
							 tf2genenodes[zz].left, tf2genenodes[zz].right,
							 miRNA2genenodes[yy].left, miRNA2genenodes[yy].right,
							  miRNA2tfnodes[xx].right, miRNA2tfnodes[xx].left
